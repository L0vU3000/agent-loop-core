...
const P = 'agent-control-plane/pipelines/technical-plan'
...
const TICKET = (args || '').replace(/\s*--provider=\S+/, '').trim()
  || '(no ticket path passed — read the newest agent-control-plane/orchestrator/inbox/*.md with type: technical-plan)'
const MAX = 3
